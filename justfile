...
dev:
    RUSTUP_TOOLCHAIN=stable CARGO_TARGET_AARCH64_APPLE_DARWIN_LINKER="{{justfile_directory()}}/scripts/sign-link.sh" corepack pnpm@10.12.4 tauri dev

# ordinary app with an OS-assigned free Vite port injected into Tauri's devUrl.
dev-auto:
    RUSTUP_TOOLCHAIN=stable CARGO_TARGET_AARCH64_APPLE_DARWIN_LINKER="{{justfile_directory()}}/scripts/sign-link.sh" node "{{justfile_directory()}}/scripts/0_tauri-dev-auto.mjs"
...
dev-safe:
    RUSTUP_TOOLCHAIN=stable INSTANT_NO_GLOBALS=1 INSTANT_TMUX_SOCKET=instant-dev-safe CARGO_TARGET_AARCH64_APPLE_DARWIN_LINKER="{{justfile_directory()}}/scripts/sign-link.sh" corepack pnpm@10.12.4 tauri dev --config '{"build":{"beforeDevCommand":"corepack pnpm@10.12.4 run dev --port 1423","devUrl":"http://localhost:1423"}}'
...
dev-isolated:
    RUSTUP_TOOLCHAIN=stable INSTANT_ISOLATED=1 INSTANT_DIRECT_PTY=1 CARGO_TARGET_AARCH64_APPLE_DARWIN_LINKER="{{justfile_directory()}}/scripts/sign-link.sh" corepack pnpm@10.12.4 tauri dev --config '{"build":{"beforeDevCommand":"corepack pnpm@10.12.4 run dev --port 1422","devUrl":"http://localhost:1422"}}'
