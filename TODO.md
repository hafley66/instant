...
- [todo(lifecycle): add isolated plugin activation and epic teardown after Status proves the runtime](src/plugin.tsx#L386)
...
- [todo(lifecycle): make child-process termination and reader-thread shutdown explicit](src-tauri/src/pty.rs#L729)
- [todo(lifecycle): move PTY listener ownership and teardown into the reactive runtime](src/terminal.ts#L930)
- [todo(migration): delete the legacy vanilla table after remaining consumers move to TreeTable](src/table.ts#L271)
- [todo(migration): version persisted state migrations explicitly before adding more fields](src/state.ts#L494)
- [todo(security): cap ingest request bodies and define localhost authentication policy](src-tauri/src/activity.rs#L837)
...
- [todo(split): move each panel into its own file and leave shared row models here](src/tablepanels.tsx#L1128)
- [todo(split): reduce the Tauri composition root to adapter registration and boot wiring](src-tauri/src/lib.rs#L913)
- [todo(split): separate Claude and OpenCode parsing from ledger query orchestration](src-tauri/src/ledger.rs#L810)
- [todo(split): separate screencast rendering, input translation, and navigation state](src/cdp.ts#L835)
- [todo(split): separate tmux commands, PTY ownership, and rogue-process discovery](src-tauri/src/pty.rs#L728)
- [todo(state): make dock layout recovery a typed state transition instead of boot-order mutation](src/reactdock.tsx#L791)
- [todo(state): split durable settings from runtime mirrors and ephemeral UI state](src/state.ts#L493)
- [todo(test): add a boot smoke test that asserts registration and teardown order](src/main.ts#L6)
- [todo(test): add fixture coverage for malformed and partially-written session logs](src-tauri/src/ledger.rs#L811)
- [todo(test): add keyboard-navigation integration coverage for every TreeTable panel](src/tablepanels.tsx#L1129)
- [todo(test): cover Worktrees panel refresh plus SSE reconnect as an integration flow](src/worktrees.ts#L8)
- [todo(test): cover corrupt layout recovery and panel remount behavior](src/reactdock.tsx#L792)
- [todo(test): cover engine crash, reconnect, and orphan cleanup](src-tauri/src/cdp.rs#L624)
- [todo(test): exercise open, resize, close, reload, and tmux reattach as one lifecycle test](src/terminal.ts#L931)
- [todo(test): verify abort, invalid JSON, empty body, and non-2xx transport behavior](src/reactive/httpTransport.ts#L32)
- [todo(test): verify one crashing plugin cannot interrupt sibling registration or rendering](src/plugin.tsx#L387)
<!-- END: todo-ready -->
