...
- [todo(boundary): replace curl-based ntfy publishing with a typed outbound notification port](src-tauri/src/activity.rs#L774)
- [todo(codegen): verify ipc/commands.json against generate_handler registrations during build](src-tauri/src/lib.rs#L881)
- [todo(http): describe /config and /ingest completely in the OpenAPI document](src-tauri/src/activity.rs#L772)
- [todo(http): support media-type-aware decoding instead of assuming JSON for every response](src/reactive/httpTransport.ts#L24)
...
- [todo(security): cap ingest request bodies and define localhost authentication policy](src-tauri/src/activity.rs#L773)
...
- [todo(test): verify abort, invalid JSON, empty body, and non-2xx transport behavior](src/reactive/httpTransport.ts#L25)
...
<!-- BEGIN: todo-blocked -->
- [todo(http): consume generated activity config and ingest contracts (depends: activity :8787 OpenAPI paths)](extension/src/background.ts#L7)
- [todo(http): consume the generated activity ingest contract (depends: activity :8787 OpenAPI paths)](vscode-ext/src/extension.ts#L9)
