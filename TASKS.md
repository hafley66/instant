...
- [x] **OS drag-drop of files/screenshots — REVERTED.** `dragDropEnabled:true` swallows the HTML5 drag dockview needs for tabs (macOS WKWebView; binary, no runtime toggle). Tab dragging wins. `dragDropEnabled` back to `false`, `wireDragDrop` removed. File attach goes through the Shot/screenshot feature.
