...
- [~] **Migrate ALL lists to TanStack `@tanstack/react-table` + `@tanstack/react-virtual`.** One headless `<TreeTable>` (columns + `getSubRows` + sort + virtual) replaces `table.ts` (`renderTable`/`virtualTable`), the `treeNode` tree, and the `<ul>` session list. Headless = keep xp/p5/ac3 CSS. Drives: worktrees tree+table, sessions, files, activity, sprefa results.
  ...
  - [ ] **Replace v1 tmux + worktrees panels with v2, keeping the v1 names** (`sessions`/`worktrees`, not `tmux2`/`wt2`), and delete the `tmux2`/`worktrees2` panels (rail currently shows 4 entries; want 2). CATCH: the v1 panels carry controls the v2 components don't — tmux: sort `<select>`, +claude/+opencode quick-launch, new-shell form; worktrees: scan-root input + Scan, Tree/Table toggle, Focus toggle, agents editor. Two paths: (a) port those controls into the React components (full innerHTML kill, more work — needs bridge methods for quickLaunch/newShell/setSort/scan/toggleFocus; agents-edit already reachable via worktree right-click "edit agents…", and new-shell via ⌘T), or (b) hybrid: keep the v1 panel html chrome, swap only the list container to a `createRoot(...).render(<TmuxPanelV2/>)` mount. Recommend (a) for the migration goal; (b) is lower-risk/less-code. Decide before starting.
  - [ ] **files v2** — tree via row-expand (`getSubRows` + the `tree` column twisty). The `noRowClick`/twisty plumbing is in place.
...
- [ ] **UI zoom in/out.** User wants native browser/text zoom (or at least tmux zoom). Best = `getCurrentWebview().setZoom(factor)` (zooms everything incl. xterm) + keybinds cmd+`=` / cmd+`-` / cmd+`0`. Persist a `zoom` field in the store; clamp ~0.5–2.0; apply on boot. Add as keymap commands (like `app.reload`). Import `getCurrentWebview` from `@tauri-apps/api/webview`.
