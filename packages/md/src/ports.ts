...
export interface MdviewPanZoomProps {
  children: ReactNode;
  controls?: (api: MdviewPanZoomApi) => ReactNode;
  className?: string;
}

export type MdviewProbeDetail = Readonly<Record<string, boolean | number | string>>;
...
export interface MdviewHost {
  ...
  PanZoomViewport: ComponentType<MdviewPanZoomProps>;

  // ---- optional host diagnostics ----
  useRenderProbe(name: string, scope?: string, detail?: MdviewProbeDetail): void;
  useLifecycleProbe(name: string, scope?: string, detail?: MdviewProbeDetail): void;
  recordOperation(name: string, detail?: MdviewProbeDetail): void;
