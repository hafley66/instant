...
| port lanes | all five merged: term-basics 17 pass 1 fixme, cmdclick 25 pass 3 fixme, hover 21 pass, diagrams 24 pass 1 fixme, panels-live 6 pass 4 fixme | `e2e-real/*.spec.ts`, PNGs under `artifacts/real/` |
| shell lane | merged: tray, ⌘⌥Space shortcut, window; the summon gestures (double right-click, double right-⌘) on a listen-only tap; capture triggers, key grab and the wdio native tier gone | `src-tauri/src/lib.rs` `spawn_summon_tap`, 80 Rust tests |
| deletions | `browserE2eInvoke`, 16 `e2e-*.html`, `e2e/*.tsx`, orphan snapshot dirs, `e2e-native/`, wdio deps | tsc clean, vite build, 625 vitest, fork spec 2 passed |
