# Lane: real-term-hover (instant, Playwright real tier)
...
- e2e/term-cmd-hover.spec.ts (8)
- e2e/term-context-hover.spec.ts (2)
- e2e/term-context-queue.spec.ts (3)
- e2e/term-context-checkbox.spec.ts (6)

Your port: `47802`. Your tmux socket: `instant-real-b`. Nobody else uses them.

## Hints for these files
- ⌘-hover: `page.keyboard.down('Meta')` then two `page.mouse.move` calls on the cell; the card is `.term-inspector` (see the fixture spec for the assertions on it). Paths in the pane text must exist on disk for the resolver to name them: create them under a `mkdtempSync` dir and `cd` the pane there first (`settleCwd`).
- `__cmdClickEvents` was a fixture log of the click resolver's route. The real receipt is what opens: a preview panel (`.mdview-content`, monaco, pdf) in dockview, or nothing. Assert on the panel.
- Context checkbox and queue lived on a `harness=codex` fixture; use the stub-harness trick from law 4. `__visibleTurnEvents` was the turn-visibility log; the real receipt is the turn sidebar and turn markers in the DOM (`src/0_terminalTurnVisibility.ts`, `src/00_terminalTurnRegions.ts` name the classes).

## Commit subject (exact)
`real tier: cmd-hover, context hover, context queue and context checkbox ported`
