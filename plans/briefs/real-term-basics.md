# Lane: real-term-basics (instant, Playwright real tier)
...
- e2e/term-selection-pin.spec.ts (4)
- e2e/term-word-select.spec.ts (5)
- e2e/term-input.spec.ts (1)
- e2e/term-sidebar.spec.ts (1)
- e2e/term-row-offset.spec.ts (2)
- e2e/term-wrap-hover.spec.ts (4)
- e2e/term-structured.spec.ts (cases live inside a describe)

Your port: `47801`. Your tmux socket: `instant-real-a`. Nobody else uses them.

## Hints for these files
- The fixture put xterm on the alternate screen with mouse tracking on (DECSET 1049/1000/1002/1006) to mimic a codex pane. In a real pane, `printf '\\e[?1049h\\e[?1006h\\e[?1000h\\e[?1002h'` from the shell does the same; tmux forwards those modes. `less` or `vim` in the pane is the other way to own the mouse.
- Pinned selection receipts: the pinned overlay rects are DOM (`.term-pin` or the class used in `src/0_terminalPinnedSelection.ts`; read that file for the class names) and the clipboard (`navigator.clipboard.readText()`, permissions are already granted in the config).
- Row offset and wrap hover: the terminal's size is the dockview panel's; resize the browser viewport with `page.setViewportSize` and let the app refit, then read `screenRows`.
- Input: keys typed with `page.keyboard.type` into the focused terminal land in the pane; `paneScreen` proves it.

## Commit subject (exact)
`real tier: terminal basics ported, selection pin, word select, input, sidebar, row offset, wrap hover, structured`
