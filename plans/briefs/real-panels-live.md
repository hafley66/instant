# Lane: real-panels-live (instant, Playwright real tier)
...
- e2e/paint-panel.spec.ts (4)
- e2e/metrics-dashboard.spec.ts (1)
- e2e-live/strip-live.live.ts (4)
- e2e-live/external-shells.live.ts (1)

Your port: `47805`. Your tmux socket: `instant-real-e`. Nobody else uses them.

## Hints for these files
- Paint and the metrics dashboard are rail panels (`#paint-toggle`, the Status or Boop panel; `src/panels.ts` and `src/plugin.tsx` list ids). Open them with the rail button, drive them with mouse and keyboard, read the canvas or table DOM.
- strip-live and external-shells ran boop lanes on the default tmux socket with a per-test mail dir and exposed tmux to the page through a Playwright binding (`e2e-live/0_live.ts`). In the real tier the backend already talks to tmux and boop; keep the node-side setup (`tmux new-session -d` on the default socket, `boop beep lane patch --mail-dir`) and drop the page binding. The in-tab strip summons with Cmd+Shift+Period; its test ids are in `src/` (grep `in-tab-strip`). The app's own boop store is `~/.agent/boop.db`; a per-test `--mail-dir` only isolates the CLI side, so name every lane with the `proof-` prefix and delete it in `afterEach` (`boop beep lane delete <lane>`).
- The Boop viewer tab attaches to a lane's pane on the default socket; `tmux list-clients -t =<session>` on the default socket counts attachments.

## Commit subject (exact)
`real tier: paint panel, metrics dashboard, strip live and external shells ported`
