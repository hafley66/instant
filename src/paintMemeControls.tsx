...
const blank = (id: CaptionRow["id"], label: string): CaptionRow => ({ id, label, text: "", family: "Impact", size: 56, bold: false, italic: false, underline: false, strikethrough: false, fill: "#ffffff", stroke: "#000000", strokeWidth: 4 });
...
export function PaintMemeControls({ bridge }: { bridge: PaintBridge | null }) {
  ...
  const apply = async () => { try { setError(""); await bridge?.applyMemeCaptions(rows[0], rows[1]); } catch (cause) { setError(String(cause)); } };
  return <aside className="paint-meme-controls" data-testid="meme-captions"><div className="meme-layers-bar"><span>meme captions</span><button type="button" disabled={!bridge || !rows.some((row) => row.text.trim())} onClick={() => void apply()}>add caption layers</button></div><TreeTable columns={columns} data={rows} getRowId={(row) => row.id} rowClass={(row) => `paint-caption-${row.id}`} />{error ? <output className="paint-caption-error">{error}</output> : null}</aside>;
}
