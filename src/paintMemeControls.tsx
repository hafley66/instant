...
export function PaintMemeControls({ bridge }: { bridge: PaintBridge | null }) {
  ...
  const columns = useMemo<TreeColumn<CaptionRow>[]>(() => [
    ...
    { id: "text", header: "text", noRowClick: true, cell: (row) => <textarea aria-label={`${row.label} text`} rows={2} value={row.text} onClick={stop} onChange={(event) => update(row.id, { text: event.currentTarget.value })} /> },
